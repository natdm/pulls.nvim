...
function M.run_job(obj, callback)
    ...
    if obj.writer then s.stdin = uv.new_pipe(false) end

    s.handle, s.pid = uv.spawn(obj.command, {args = obj.args, stdio = {s.stdin, s.stdout, s.stderr}, cwd = obj.cwd}, function(code, signal)
        s.code = code
        s.signal = signal
    ...
    if not s.handle then error(debug.traceback("Failed to spawn process: " .. vim.inspect(obj))) end
